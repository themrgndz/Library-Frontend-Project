...
const Navbar = ({ onSearch }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const booksResponse = await fetch('http://192.168.1.184:5001/api/books');
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch('http://192.168.1.184:5001/api/book', {
        method: 'POST',
  ...
  const handleBorrowClick = () => {
    window.location.href = 'http://192.168.1.184:5001/borrow';
  };
